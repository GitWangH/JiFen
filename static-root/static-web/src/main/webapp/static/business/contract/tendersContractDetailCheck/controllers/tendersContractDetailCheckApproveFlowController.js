angular.module('huatek.controllers').controller('TendersContractDetailCheckApproveFlowController', function ($scope, $http, editService, httpService, treeGridService) {
	...
	/*定义块*/
    var columnWrapArray = [];
    columnWrapArray.push(new multipleColumn(1,'合同复核清单概况'));
    if(!$scope.onlyView){
    ...
    /**创建表格*/
    var columnsArr = [];
    columnsArr.push(new treeColumn('清单编号','contractDetailCode','15%',false ));
    columnsArr.push(new treeColumn('清单名称','contractDetailName','15%',false ));
    columnsArr.push(new treeColumn('单位', 'countUnit', '5%',true,new otherConfig('select','dic.count_unit')));
    columnsArr.push(new treeColumn('合同单价(元)','unitPrice','10%',false ));
    columnsArr.push(new treeColumn('合同数量','quantity','5%',false ));
    columnsArr.push(new treeColumn('合同总额','totalPrice','10%',false ));
    columnsArr.push(new treeColumn('复核单价','reviewUnitPrice','10%'));
    columnsArr.push(new treeColumn('复核数量','reviewQuantity','10%'));
    columnsArr.push(new treeColumn('复核总额','reviewTotalPrice','10%'));
