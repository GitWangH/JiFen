 ...
 * 右边 项目形象清单与项目分部分项
 * */
angular.module('huatek.controllers').controller('busiProgressImageListSubConnectionTableController', function ($scope, $location, $http,$rootScope,editService, listService, shareData, httpService, treeGridService) {
	/**actionUrl start*/
	/*查询顶级节点URL*/
	...
	/*右侧数据查询*/
	var rightGridQueryUrl = "/api/busiProgressImageToBranchConnect/query/";
	/*删除右侧表格选中的数据*/
	var deleteRightDataUrl = "/api/busiProgressImageToBranchConnect/delete";
    /*形象清单id 左侧选择的id*/
    var imageId = 0;
	/**actionUrl end*/
    ...
    columnsArr.push(new treeColumn('挂接分部分项','','50%',true,new otherConfig('button',null,function(row){
    	...
    	imageId = row.id;
        listService.addData($scope,new popup('项目形象清单挂接分部分项','/busiBaseConnectionSubEngineering/hitch',obj,'1200','800'));
    })));
    ...
    $scope.tableGrid = {
	    ...
	    multiSelect: true,
        columnDefs: [
            { name: '形象清单编号', field: 'imageCode', width: '24%', enableColumnMenu: false },
            { name: '形象清单名称', field: 'imageName', width: '25%', enableColumnMenu: false },
            { name: '分部分项编号', field: 'branchCode', width: '24%', enableColumnMenu: false },
            { name: '分部分项名称', field: 'branchName', width: '25%', enableColumnMenu: false }
        ]
    };
    /** 注册页面右侧的选择器 */
    ...
    btnArray.push(new pageButton('删除', 'delete', '/busiProgressImageListSubConnection/delete', 'delete'));
