...
angular.module('huatek.controllers').controller('busiBaseSubEngineeringController', function($scope, httpService, listService, treeGridService) {
    /*查询顶级节点URL*/
    ...
    /*保存修改数据RUL*/
    var saveDataUrl = '/api/busiBaseSubEngineering/saveData'

    /**创建表格*/
    var columnsArr = [];
    columnsArr.push(new treeColumn('编号', 'number', '10%'));
    columnsArr.push(new treeColumn('名称', 'name', '20%'));
    columnsArr.push(new treeColumn('单位工程类型', 'unit', '20%', true, new otherConfig('select', 'dic.unit_project_type')));
    columnsArr.push(new treeColumn('备注', 'remark', '50%'));
    /*    columnsArr.push(new treeColumn('操作','remark','30%',true,new otherConfig('button',null,function(row){
        })));*/
    ...
    /*定义功能按钮*/
    var btnArray = [];
    btnArray.push(new pageButton('保存', 'save', '/busiBaseSubEngineering/save', 'save'));
    /* 设置界面的功能按钮.*/
