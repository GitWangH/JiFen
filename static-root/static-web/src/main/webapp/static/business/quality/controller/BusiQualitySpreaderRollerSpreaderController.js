/**
 * 路面摊压检测Controller 
 * BusiQualitySpreaderRollerSpreaderController
 */
'use strict';
angular.module('huatek.controllers').controller('BusiQualitySpreaderRollerSpreaderController', function ($rootScope, $scope, $location, $http, listService,httpService) {
	...
	$scope.tableGrid = {
	        ...
	        columnDefs: [
				...
				{ name: '超标等级', field: 'overproofGrade_',width: '10%', enableColumnMenu: false,decode:{field:'overproofGrade',dataKey:'dic.excessive_grade'}},
				...
				{ name: '检测状态', field: 'dataType_',width: '10%', enableColumnMenu: false,decode:{field:'dataType',dataKey:'dic.is_qualified'}},
	    ...
	    var type = new queryParam('作业类型','type','string','=','','dic.job_type');
	    type.componentType = 'select';
	    queryPage.addParam(type);
	    var dataType = new queryParam('检测状态','dataType','string','=','1','dic.is_qualified');
	    dataType.componentType = 'select';
	    queryPage.addParam(dataType);
	    var overproofGrade = new queryParam('超标等级','overproofGrade','string','=',null,'dic.excessive_grade');
	    ...
	    // 定义功能按钮
		var btnArray = [];
		/*btnArray.push(new pageButton('查看', 'view', '/busiQualitySpreaderRollerSpreader/view', 'viewData'));*/
	    btnArray.push(new pageButton('快捷处理','quickProcess','/busiQualitySpreaderRollerSpreader/quickProcess','quickProcessData'));
	    btnArray.push(new pageButton('整改下发','rectification','/busiQualitySpreaderRollerSpreader/rectification','rectificationData'));
	    btnArray.push(new pageButton('查看审批流','process','/busiQualitySpreaderRollerSpreader/process','processData'));
		/**
		 * 设置界面的功能按钮.
		 */
	    ...
	    $scope.execute = function(operation, param){
	        ...
	        if(operation == 'viewData'){
	        	if(param.disposeState=='5' || param.disposeState=='4'){
	        		listService.editData($scope,$scope.gridApi, new popup("查看","/busiQualitySpreaderRollerSpreader/view", 'spreader_roller', "1200", "800"),param);
	        	}else if(param.disposeState=='1'||param.disposeState=='2'||param.disposeState=='6'){
	        		listService.editData($scope,$scope.gridApi, new popup("查看","/busiQualityQuickProcess/detail", 'spreader_roller', "1200", "800"),param);
	        	}else{
	        	...
	        	if(isAdminFn()){
	        		...
	        		for(var i=0;i<selectData.length;i++){
	        			if (selectData[i].disposeState!='0' && selectData[i].disposeState!='6' || selectData[i].dataType!='1') {
		 					submitTips('该条数据已提交处理或为合格数据，不能快捷处理！', 'warning');
		 					ids = [];
		 					break;
	        			}
	        			
	        			if(selectData[i].overproofGrade=='2' && Number($rootScope.orgType)>6){
	        				boo = true;
		        			break;
		        		}
		        		if(selectData[i].overproofGrade=='3' && Number($rootScope.orgType)>4){
		        			boo = true;
		        			break;
		        		}
	        		...
	        		if(boo){
	        			...
	        			if(ids.length>0){
	        				...
	        				listService.addData($scope,new popup("快捷处理","/busiQualitySpreaderRollerSpreader/quickProcess", map, "900", "600"));
	        			}
	        		}
	        	}
	        	...
	        	if(isAdminFn()){
	        		...
	        		var ids = [];
	        		for(var i=0;i<selectData.length;i++){
	        			if (selectData[i].disposeState!='0' || selectData[i].dataType!='1') {
	        		...
	        		if(ids.length>0){
	        			...
	        			listService.addData($scope,new popup("整改下发","/busiQualitySpreaderRollerSpreader/rectification", map, "1200", "800"));
	        		}
	        	}
	    ...
	    var load = $scope.load = function() {
	        listService.loadData($scope, '/api/busiQualitySpreaderRollerSpreader/query', $scope.tableGrid);
	    }
 ...
 * 查询整改明细
 */
angular.module('huatek.controllers').controller('BusiQualitySpreaderRollerSpreaderDetailController', function ($scope,$rootScope, $location, $http,listService, editService, cacheService,httpService,$q) {
	var originType = $scope.passParams;
	var typeField = '';
	/* 判断接口来源类型 */
	if(originType=='spreader_roller'){
		typeField = [
			{ name: '标段名称', field: 'orgName',width: '10%', enableColumnMenu: false},
			{ name: '作业层面', field: 'operationSurface',width: '10%', enableColumnMenu: false},
			{ name: '采集时间', field: 'gatherDate',width: '10%', enableColumnMenu: false},
...
angular.module('huatek.controllers').controller('BusiQualitySpreaderRollerSpreaderRectificationController', function ($scope, $location, $http, editService, cacheService,$rootScope) {
	var addDataUrl = '/api/busiQualitySpreaderRollerSpreader/rectificationAdd';
	var homeUrl = "/busiQualitySpreaderRollerSpreader/home";
    /*定义块*/
    var columnWrapArray = [];
    ...
    contractFileUUID.systemHeader = "/root/quality/semifinished/spreaderRoller";
 ...
 * 路面摊压检测快捷处理
 */
angular.module('huatek.controllers').controller('BusiQualitySpreaderRollerSpreaderQuickProcessController', function ($rootScope, $scope, $http, listService, editService,httpService) {
	//console.log($scope.passParams)
	var addDataUrl = '/api/busiQualitySpreaderRollerSpreader/quickProcessAdd';
    ...
    */
	 ...
	 * 初始化加载数据.
	 */
    /*httpService.get($scope,'/api/busiQualitySpreaderRollerSpreader/queryList'+"/"+$scope.passParams.id).success(function(response){
    	$scope.tableGrid.data = response;
    });*/
