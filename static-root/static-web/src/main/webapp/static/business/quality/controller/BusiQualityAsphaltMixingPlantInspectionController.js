/**
 * 沥青拌合站检测Controller 
 * BusiQualityAsphaltMixingPlantController
 */
'use strict';
angular.module('huatek.controllers').controller('BusiQualityAsphaltMixingPlantController', function ($rootScope, $scope, $location, $http, listService,httpService) {
	...
	$scope.tableGrid = {
	        ...
	        columnDefs: [
				...
				{ name: '检测状态', field: 'isExcessive1',width: '11%', enableColumnMenu: false,decode:{field:'isExcessive',dataKey:'dic.is_excessive'}},
				{ name: '处理状态', field: 'disposeState',width: '11%', enableColumnMenu: false,decode:{parent:'busiQualityAsphaltMixingPlantExceed',field:'disposeState',dataKey:'dic.deal_type_new'},cellTemplate:listService.getLinkCellTmplate("viewData","disposeState")},
	        ]
	    };
		/**
	    ...
	    var overproofGrade = new queryParam('超标等级','busiQualityAsphaltMixingPlantExceed.overproofGrade','string','=',null,'dic.excessive_grade')
	    overproofGrade.componentType = 'select';
	    queryPage.addParam(overproofGrade);
	    var disposeState = new queryParam('处理状态','busiQualityAsphaltMixingPlantExceed.disposeState','string','=',null,'dic.deal_type_new')
	    ...
	    // 定义功能按钮
		var btnArray = [];
		/*btnArray.push(new pageButton('查看', 'view', '/busiQualityAsphaltMixingPlant/view', 'viewData'));*/
	    btnArray.push(new pageButton('快捷处理','quickProcess','/busiQualityAsphaltMixingPlant/quickProcess','quickProcessData'));
	    btnArray.push(new pageButton('整改下发','rectification','/busiQualityAsphaltMixingPlant/rectification','rectificationData'));
	    btnArray.push(new pageButton('查看审批流','process','/busiQualityAsphaltMixingPlant/process','processData'));
		/**
		 * 设置界面的功能按钮.
		 */
	    ...
	    $scope.execute = function(operation, param){
	        ...
	        if(operation == 'viewData'){
	        	if(param.busiQualityAsphaltMixingPlantExceed.disposeState=='5' || param.busiQualityAsphaltMixingPlantExceed.disposeState=='4'){
	        		listService.editData($scope,$scope.gridApi, new popup("查看","/busiQualityAsphaltMixingPlant/view", 'asphalt_mixing_station', "1200", "800"),param);
	        	}else if(param.busiQualityAsphaltMixingPlantExceed.disposeState=='1'||param.busiQualityAsphaltMixingPlantExceed.disposeState=='2'||param.busiQualityAsphaltMixingPlantExceed.disposeState=='6'){
	        		listService.editData($scope,$scope.gridApi, new popup("查看","/busiQualityQuickProcess/detail", 'asphalt_mixing_station', "1200", "800"),param);
	        	}else{
	        	...
	        	if(isAdminFn()){
	        		...
	        		for(var i=0;i<selectData.length;i++){
	        			...
	        			if(selectData[i].busiQualityAsphaltMixingPlantExceed && selectData[i].busiQualityAsphaltMixingPlantExceed.disposeState!='0'&& selectData[i].busiQualityAsphaltMixingPlantExceed.disposeState!='6'){
	        				submitTips('该条数据已提交处理,请选择未处理的数据！','warning');
	        				return;
	        			}
	        			if(selectData[i].busiQualityAsphaltMixingPlantExceed){
	        				if(selectData[i].busiQualityAsphaltMixingPlantExceed.overproofGrade=='2' && Number($rootScope.orgType)>6){
	        					boo = true;
	        					break;
	        				}
	        				if(selectData[i].busiQualityAsphaltMixingPlantExceed.overproofGrade=='3' && Number($rootScope.orgType)>4){
	        					boo = true;
	        					break;
	        				}
	        			}else{
	        		...
	        		if(boo){
	        			...
	        			listService.addData($scope,new popup("快捷处理","/busiQualityAsphaltMixingPlant/quickProcess", map, "900", "600"));
	        		}
	        	}
	        	...
	        	if(isAdminFn()){
	        		...
	        		if(selectData[0].busiQualityAsphaltMixingPlantExceed.disposeState!='0'){
	        			submitTips('该条数据已提交处理,请选择未处理的数据！','warning');
	        			return;
	        		}
	        		...
	        		listService.addData($scope,new popup("整改下发","/busiQualityAsphaltMixingPlant/rectification", map, "1200", "800"));
	        	}
	        	...
	        	if(selectedData){
	        		if(selectedData.busiQualityAsphaltMixingPlantExceed.disposeState=='0' || selectedData.busiQualityAsphaltMixingPlantExceed.disposeState=='3'){
	        			submitTips('该条数据还未处理或未整改！','warning');
	        			return;
	        		}
	        		if(selectedData.busiQualityAsphaltMixingPlantExceed.disposeState=='2' || selectedData.busiQualityAsphaltMixingPlantExceed.disposeState=='1'|| selectedData.busiQualityAsphaltMixingPlantExceed.disposeState=='6'){/*处理流程*/
	        		...
	        		}else if(selectedData.busiQualityAsphaltMixingPlantExceed.disposeState=='4' || selectedData.busiQualityAsphaltMixingPlantExceed.disposeState=='5'){/*整改流程*/
	        	...
	        	window.open(param['busiQualityAsphaltMixingPlantExceed']['reportAddress'],"检测报告");
	    	}
	    }
	    /**
		 * 初始化加载数据.
	    ...
	    var load = $scope.load = function() {
	        listService.loadData($scope, '/api/busiQualityAsphaltMixingPlant/query', $scope.tableGrid,null,function(res){
	        	if(res.content && res.content.length > 0){
	        		for(var i = 0 ; i < res.content.length ; i ++){
	        			if(res.content[i].isExcessive == '1' && res.content[i].busiQualityAsphaltMixingPlantExceed){
	        				res.content[i].createTime = res.content[i].busiQualityAsphaltMixingPlantExceed.createTime;
	        			}/*else{
 ...
 * 查询整改明细
 */
angular.module('huatek.controllers').controller('BusiQualityAsphaltMixingPlantDetailController', function ($scope,$rootScope, $location, $http,listService, editService, cacheService,httpService,$q) {
	var originType = $scope.passParams;
	var typeField = '';
	/* 判断接口来源类型 */
	if(originType=='asphalt_mixing_station'){
		typeField = [
			{ name: '施工地点', field: 'constructionPosition',width: '12%', enableColumnMenu: false},
			{ name: '工程名称', field: 'engineeringName',width: '12%', enableColumnMenu: false},
			{ name: '配方号', field: 'formula',width: '12%', enableColumnMenu: false},
...
angular.module('huatek.controllers').controller('BusiQualityAsphaltMixingPlantRectificationController', function ($scope, $location, $http, editService, cacheService,$rootScope) {
	var addDataUrl = '/api/busiQualityAsphaltMixingPlant/rectificationAdd';
	var homeUrl = "/busiQualityAsphaltMixingPlant/home";
    /*定义块*/
    var columnWrapArray = [];
    ...
    contractFileUUID.systemHeader = "/root/quality/semifinished/asphaltMixingPlant";
 ...
 * 沥青拌和检测快捷处理
 */
angular.module('huatek.controllers').controller('BusiQualityAsphaltMixingPlantQuickProcessController', function ($rootScope, $scope, $http, listService, editService,httpService) {
	console.log($scope.passParams)
	var addDataUrl = '/api/busiQualityAsphaltMixingPlant/quickProcessAdd';
    ...
    */
	 ...
	 * 初始化加载数据.
	 */
   /* httpService.get($scope,'/api/busiQualityAsphaltMixingPlant/queryList'+"/"+$scope.passParams.id).success(function(response){
    	$scope.tableGrid.data = response;
    });*/
