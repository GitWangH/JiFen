/**
 * 初期支护净空断面检测Controller 
 * BusiQualityPrimarySupportClearanceSectionCheckController
 */
'use strict';
angular.module('huatek.controllers').controller('BusiQualityPrimarySupportClearanceSectionCheckController', function ($rootScope, $scope, $location, $http, listService) {
	...
	$scope.tableGrid = {
	        ...
	        columnDefs: [
				...
				{ name: '断面桩号', field: 'sectionStakeNo',width: '10%', enableColumnMenu: false},
				{ name: '是否侵线', field: 'invasionLineStatus1',width: '10%', enableColumnMenu: false,decode:{field:'invasionLineStatus',dataKey:'dic.invasion_line_status'}},
				{ name: '衬砌类型', field: 'liningType',width: '10%', enableColumnMenu: false},
				{ name: '侵线最大值(cm)', field: 'invasionLineMaxLength',width: '10%', enableColumnMenu: false},
				{ name: '侵线位置', field: 'invasionLinePosition',width: '10%', enableColumnMenu: false},
				{ name: '检测开始时间', field: 'startCheckDate',width: '10%', enableColumnMenu: false},
				{ name: '检测结束时间', field: 'endCheckDate',width: '9%', enableColumnMenu: false},
				...
				{ name: '修改日志', field: 'modifyTime',width: '10%', enableColumnMenu: false,cellTemplate:listService.getLinkCellTmplate("queryModifyLog","modifyTime","日志")},
	        ]
	    };
		/**
	    ...
	    // 定义功能按钮
		var btnArray = [];
	    btnArray.push(new pageButton('新增','add','/primarySupportClearanceSectionCheck/add','addData'));
	    btnArray.push(new pageButton('修改','edit','/primarySupportClearanceSectionCheck/edit','editData'));
	    /*btnArray.push(new pageButton('查看', 'view', '/primarySupportClearanceSectionCheck/view', 'viewData'));*/
		/**
		 * 设置界面的功能按钮.
		 */
	    ...
	    $scope.execute = function(operation, param){
	        ...
	        if(operation == 'addData'){
	        	if(isAdminFn()){
	        		listService.addData($scope, new popup("新增","/primarySupportClearanceSectionCheck/add", null, "1200", "800"));
	        	}
	        }else if(operation == 'editData'){
	        	if(isAdminFn()){
	        		listService.editData($scope, $scope.gridApi, new popup("修改","/primarySupportClearanceSectionCheck/edit", null, "1200", "800"));
	        	}
	        }else if(operation == 'viewData'){
	        	listService.editData($scope, $scope.gridApi, new popup("查看","/primarySupportClearanceSectionCheck/view", null, "1200", "800"),param);
	        	...
	        	var logShowFieldArr = [
					...
					{ name: '侵线最大值(cm)', field: 'invasionLineMaxLength',width: '10%', enableColumnMenu: false},
					{ name: '侵线位置', field: 'invasionLinePosition',width: '10%', enableColumnMenu: false},
					{ name: '检测开始时间', field: 'startCheckDate',width: '10%', enableColumnMenu: false, cellFilter: "dateFilter" },
					...
					{ name: '修改时间', field: 'modifyTime',width: '15%', enableColumnMenu: false},
	   	        	...
	   	        	var queryLogUrl = '/api/primarySupportClearanceSectionCheck/queryModifyLog/'+param.id;
	    ...
	    var load = $scope.load = function() {
	        listService.loadData($scope, '/api/primarySupportClearanceSectionCheck/query', $scope.tableGrid);
	    }
...
angular.module('huatek.controllers').controller('BusiQualityPrimarySupportClearanceSectionCheckAddController', function ($scope, $http, editService, httpService,$rootScope,listService) {
	var addDataUrl = 'api/primarySupportClearanceSectionCheck/add';
	var editDataUrl = 'api/primarySupportClearanceSectionCheck/edit';
	var homeUrl = '/primarySupportClearanceSectionCheck/home';
    ...
    contractFileUUID.systemHeader = "/root/quality/thirdPartyInspection/clearanceSection";
...
/*查看明细*/
angular.module('huatek.controllers').controller('BusiQualityPrimarySupportClearanceSectionCheckDetailController', function ($scope, $http, editService, httpService,$rootScope) {
	var editDataUrl = 'api/primarySupportClearanceSectionCheck/edit';
	var homeUrl = '/primarySupportClearanceSectionCheck/home';
