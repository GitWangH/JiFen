/**
 * 水稳拌合站检测Controller 
 * BusiQualityWaterStableMixingStationController
 */
'use strict';
angular.module('huatek.controllers').controller('BusiQualityWaterStableMixingStationController', function ($rootScope, $scope, $location, $http, listService,httpService) {
	...
	$scope.tableGrid = {
	        ...
	        columnDefs: [
				...
				{ name: '上传时间', field: 'createTime',width: '11%', enableColumnMenu: false},
				{ name: '检测状态', field: 'isExcessive_',width: '11%', enableColumnMenu: false,decode:{field:'isExcessive',dataKey:'dic.is_excessive'}},
				{ name: '处理状态', field: 'disposeState',width: '11%', enableColumnMenu: false,decode:{parent:'busiQualityWaterStableMixingStationExceed',field:'disposeState',dataKey:'dic.deal_type_new'},cellTemplate:listService.getLinkCellTmplate("viewData","disposeState")},
	        ]
	    };
		/**
	   ...
	   // queryPage.addParam(new queryParam('工程名称','engineeringName','string','like'));
	    ...
	    var overproofGrade = new queryParam('超标等级','busiQualityWaterStableMixingStationExceed.overproofGrade','string','=',null,'dic.excessive_grade');
	    overproofGrade.componentType = 'select';
	    queryPage.addParam(overproofGrade);
	    var disposeState = new queryParam('处理状态','busiQualityWaterStableMixingStationExceed.disposeState','string','=',null,'dic.deal_type_new');
	    ...
	    // 定义功能按钮
		var btnArray = [];
		/*btnArray.push(new pageButton('查看', 'view', '/busiQualityWaterStableMixingStation/view', 'viewData'));*/
	    btnArray.push(new pageButton('快捷处理','quickProcess','/busiQualityWaterStableMixingStation/quickProcess','quickProcessData'));
	    btnArray.push(new pageButton('整改下发','rectification','/busiQualityWaterStableMixingStation/rectification','rectificationData'));
	    btnArray.push(new pageButton('查看审批流','process','/busiQualityWaterStableMixingStation/process','processData'));
		/**
		 * 设置界面的功能按钮.
		 */
	    ...
	    $scope.execute = function(operation, param){
	        ...
	        if(operation == 'viewData'){
	        	if(param.busiQualityWaterStableMixingStationExceed.disposeState=='5' || param.busiQualityWaterStableMixingStationExceed.disposeState=='4'){
	        		listService.editData($scope,$scope.gridApi, new popup("查看","/busiQualityWaterStableMixingStation/view", 'water_stable_mixing_station', "1200", "800"),param);
	        	}else if(param.busiQualityWaterStableMixingStationExceed.disposeState=='1'||param.busiQualityWaterStableMixingStationExceed.disposeState=='2'||param.busiQualityWaterStableMixingStationExceed.disposeState=='6'){
	        		listService.editData($scope,$scope.gridApi, new popup("查看","/busiQualityQuickProcess/detail", 'water_stable_mixing_station', "1200", "800"),param);
	        	}else{
	        	...
	        	if(isAdminFn()){
	        		...
	        		for(var i=0;i<selectData.length;i++){
	        			...
	        			if(selectData[i].busiQualityWaterStableMixingStationExceed && selectData[i].busiQualityWaterStableMixingStationExceed.disposeState!='0' && selectData[i].busiQualityWaterStableMixingStationExceed.disposeState!='6'){
	        				submitTips('该条数据已提交处理,请选择未处理的数据！','warning');
	        				return;
	        			}
	        			if(selectData[i].busiQualityWaterStableMixingStationExceed){
	        				if(selectData[i].busiQualityWaterStableMixingStationExceed.overproofGrade=='2' && Number($rootScope.orgType)>6){
	        					boo = true;
	        					break;
	        				}
	        				if( selectData[i].busiQualityWaterStableMixingStationExceed.overproofGrade=='3' && Number($rootScope.orgType)>4){
	        					boo = true;
	        					break;
	        				}
	        			}else{
	        				submitTips('配方名称为:'+selectData[i].recipeName+'的数据未同步完成！','warning');
	        				return;
	        			}
	        		...
	        		if(boo){
	        			...
	        			listService.addData($scope,new popup("快捷处理","/busiQualityWaterStableMixingStation/quickProcess", map, "900", "600"));
	        		}
	        	}
	        	...
	        	if(isAdminFn()){
	        		...
	        		if(selectData[0].busiQualityWaterStableMixingStationExceed.disposeState!='0'){
	        			submitTips('该条数据已提交处理,请选择未处理的数据！','warning');
	        			return;
	        		}
	        		var scope = $rootScope.$new();
	        		...
	        		listService.addData($scope,new popup("整改下发","/busiQualityWaterStableMixingStation/rectification", map, "1200", "800"));
	        	}
	        	...
	        	if(selectedData){
	        		if(selectedData.busiQualityWaterStableMixingStationExceed.disposeState=='0' || selectedData.busiQualityWaterStableMixingStationExceed.disposeState=='3'){
	        			submitTips('该条数据还未处理或未整改！','warning');
	        			return;
	        		}
	        		if(selectedData.busiQualityWaterStableMixingStationExceed.disposeState=='2' || selectedData.busiQualityWaterStableMixingStationExceed.disposeState=='1'){/*处理流程*/
	        		...
	        		}else if(selectedData.busiQualityWaterStableMixingStationExceed.disposeState=='4' || selectedData.busiQualityWaterStableMixingStationExceed.disposeState=='5'){/*整改流程*/
	        	...
	        	window.open(param['busiQualityWaterStableMixingStationExceed']['reportAddress'],"检测报告");
	    	}
	    }
	    /**
		 * 初始化加载数据.
	    ...
	    var load = $scope.load = function() {
	        listService.loadData($scope, '/api/busiQualityWaterStableMixingStation/query', $scope.tableGrid,null,function(res){
	        	if(res.content && res.content.length > 0){
	        		for(var i = 0 ; i < res.content.length ; i ++){
	        			if(res.content[i].isExcessive == '1' && res.content[i].busiQualityWaterStableMixingStationExceed){
	        				res.content[i].createTime = res.content[i].busiQualityWaterStableMixingStationExceed.createTime;
	        			}/*else{
 ...
 * 查询整改明细
 */
angular.module('huatek.controllers').controller('BusiQualityWaterStableMixingStationDetailController', function ($scope,$rootScope, $location, $http,listService, editService, cacheService,httpService,$q) {
	var originType = $scope.passParams;
	var typeField = '';
	/* 判断接口来源类型 */
	if(originType=='water_stable_mixing_station'){
		typeField = [
			{ name: '配方名称', field: 'recipeName',width: '11%', enableColumnMenu: false},
			{ name: '配方号', field: 'recipeCode',width: '11%', enableColumnMenu: false},
			{ name: '采集时间', field: 'gatherDate',width: '11%', enableColumnMenu: false},
...
angular.module('huatek.controllers').controller('BusiQualityWaterStableMixingStationRectificationController', function ($scope, $location, $http, editService, cacheService,$rootScope) {
	var addDataUrl = '/api/busiQualityWaterStableMixingStation/rectificationAdd';
	var homeUrl = "/busiQualityWaterStableMixingStation/home";
    /*定义块*/
    var columnWrapArray = [];
    ...
    contractFileUUID.systemHeader = "/root/quality/semifinished/waterStableMixing";
 ...
 * 水泥拌合站检测快捷处理
 */
angular.module('huatek.controllers').controller('BusiQualityWaterStableMixingStationQuickProcessController', function ($rootScope, $scope, $http, listService, editService,httpService) {
	var addDataUrl = '/api/busiQualityWaterStableMixingStation/quickProcessAdd';
    ...
    */
	 ...
	 * 初始化加载数据.
	 */
   /* httpService.get($scope,'/api/busiQualityWaterStableMixingStation/queryList'+"/"+$scope.passParams.id).success(function(response){
    	$scope.tableGrid.data = response;
    });*/
