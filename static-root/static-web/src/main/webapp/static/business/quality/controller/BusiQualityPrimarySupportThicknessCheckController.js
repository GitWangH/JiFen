/**
 * 初期支护厚度检测Controller 
 * BusiQualityPrimarySupportThicknessCheckController
 */
'use strict';
angular.module('huatek.controllers').controller('BusiQualityPrimarySupportThicknessCheckController', function ($rootScope, $scope, $location, $http, listService) {
	...
	$scope.tableGrid = {
	        ...
	        columnDefs: [
				...
				{ name: '分部分项', field: 'tendersBranchName', width: '8%', enableColumnMenu: false},
				...
				{ name: '设计厚度(cm)', field: 'designIntervalLength',width: '8%', enableColumnMenu: false},
				{ name: '实测平均厚度(cm)', field: 'actualMeasureAverageSpacing',width: '8%', enableColumnMenu: false},
				{ name: '合格率(%)', field: 'qualifiedRate',width: '8%', enableColumnMenu: false},
				...
				{ name: '是否合格', field: 'qualifiedStatus',width: '5%', enableColumnMenu: false,decode:{field:'qualifiedStatus',dataKey:'dic.inspection_status'}},
				{ name: '修改日志', field: 'modifyTime',width: '5%', enableColumnMenu: false,cellTemplate:listService.getLinkCellTmplate('queryModifyLog','modifyTime','日志')}
			]
	    };
		/**
	    ...
	    // 定义功能按钮
		var btnArray = [];
	    btnArray.push(new pageButton('新增','add','/primarySupportThicknessCheck/add','addData'));
	    btnArray.push(new pageButton('修改','edit','/primarySupportThicknessCheck/edit','editData'));
	    /*btnArray.push(new pageButton('查看', 'view', '/primarySupportThicknessCheck/view', 'viewData'));*/
		/**
		 * 设置界面的功能按钮.
		 */
	    ...
	    $scope.execute = function(operation, param){
	        ...
	        if(operation == 'addData'){
	        	if(isAdminFn()){
	        		listService.addData($scope, new popup("新增","/primarySupportThicknessCheck/add", null, "1200", "800"));
	        	}
	        }else if(operation == 'editData'){
	        	if(isAdminFn()){
	        		listService.editData($scope, $scope.gridApi, new popup("修改","/primarySupportThicknessCheck/edit", null, "1200", "800"));
	        	}
	        }else if(operation == 'viewData'){
	        	listService.editData($scope, $scope.gridApi, new popup("查看","/primarySupportThicknessCheck/view", null, "1200", "800"),param);
	        	...
	        	var logShowFieldArr = [
					...
					{ name: '起讫桩号', field: 'beginToEndStakeNo',width: '9%', enableColumnMenu: false},
					{ name: '区间长度(m)', field: 'intervalLength',width: '9%', enableColumnMenu: false},
					{ name: '衬砌类型', field: 'liningType',width: '9%', enableColumnMenu: false},
					{ name: '设计厚度(cm)', field: 'designIntervalLength',width: '9%', enableColumnMenu: false},
					{ name: '实测平均厚度(cm)', field: 'actualMeasureAverageSpacing',width: '9%', enableColumnMenu: false},
					{ name: '合格率(%)', field: 'qualifiedRate',width: '9%', enableColumnMenu: false},
					{ name: '检测开始时间', field: 'startCheckDate',width: '8%', enableColumnMenu: false,cellFilter: "dateFilter" },
					{ name: '检测结束时间', field: 'endCheckDate',width: '8%', enableColumnMenu: false,cellFilter: "dateFilter" },
					{ name: '修改时间', field: 'modifyTime',width: '15%', enableColumnMenu: false}
   		        ];
   	        	var queryLogUrl = '/api/primarySupportThicknessCheck/queryModifyLog/'+param.id;
	    ...
	    var load = $scope.load = function() {
	        listService.loadData($scope, '/api/primarySupportThicknessCheck/query', $scope.tableGrid);
	    }
...
angular.module('huatek.controllers').controller('BusiQualityPrimarySupportThicknessCheckAddController', function ($scope, $http, editService, httpService,$rootScope,listService) {
	var addDataUrl = 'api/primarySupportThicknessCheck/add';
	var editDataUrl = 'api/primarySupportThicknessCheck/edit';
	var homeUrl = '/primarySupportThicknessCheck/home';
    ...
    formFieldArray.push(new FormElement(1,'起讫桩号','beginToEndStakeNo','string',0,'30'));
    ...
    formFieldArray.push(new FormElement(1,'设计厚度(cm)','designIntervalLength','number',0,'30'));
    formFieldArray.push(new FormElement(1,'实测平均厚度(cm)','actualMeasureAverageSpacing','number',0,'30'));
    formFieldArray.push(new FormElement(1,'合格率(%)','qualifiedRate','number',0,'30'));
    ...
    contractFileUUID.systemHeader = "/root/quality/thirdPartyInspection/thicknessCheck";
...
/*查看明细*/
angular.module('huatek.controllers').controller('BusiQualityPrimarySupportThicknessCheckDetailController', function ($scope, $http, editService, httpService,$rootScope) {
	var editDataUrl = 'api/primarySupportThicknessCheck/edit';
	var homeUrl = '/primarySupportThicknessCheck/home';
    ...
    /*定义用户录入数据FormElement(column, title, name, type, require, max, model, event, min, defaultValue, dropDataUrl, readonly)*/
    var formFieldArray = [];
    var orgId = new FormElement(1,'标段名称','orgName','string','require','50','',null,null,null,null,'readonly');
   /* editService.initFieldItems($scope, orgId,'api/org/getFwOrgByType/'+$rootScope.orgType);*//**初始化表单选择数据，包括字典数据，其他可选项数据.**//*
    ...
    formFieldArray.push(new FormElement(1,'设计厚度(cm)','designIntervalLength','number',0,'30',null,null,null,null,null,'readonly'));
    formFieldArray.push(new FormElement(1,'实测平均厚度(cm)','actualMeasureAverageSpacing','number',0,'30',null,null,null,null,null,'readonly'));
    formFieldArray.push(new FormElement(1,'合格率(%)','qualifiedRate','number',0,'30',null,null,null,null,null,'readonly'));
