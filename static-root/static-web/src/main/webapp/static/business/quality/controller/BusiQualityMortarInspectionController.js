/**
 * 砂浆检测Controller 
 * BusiQualityMortarInspectionController
 */
'use strict';
angular.module('huatek.controllers').controller('BusiQualityMortarInspectionController', function ($rootScope, $scope, $location, $http, listService,httpService) {
	...
	$scope.tableGrid = {
	        ...
	        columnDefs: [
				...
				{ name: '检测状态', field: 'isQualified1',width: '10%', enableColumnMenu: false,decode:{field:'isQualified',dataKey:'dic.is_qualified'}},
	    ...
	    queryPage.addParam(new queryParam('试验日期','testDate','date','>='));
	    queryPage.addParam(new queryParam('---','testDate','date','<'));
	    ...
	    // 定义功能按钮
		var btnArray = [];
		/*btnArray.push(new pageButton('查看', 'view', '/busiQualityMortarInspection/view', 'viewData'));*/
	    btnArray.push(new pageButton('快捷处理','quickProcess','/busiQualityMortarInspection/quickProcess','quickProcessData'));
	    btnArray.push(new pageButton('整改下发','rectification','/busiQualityMortarInspection/rectification','rectificationData'));
	    btnArray.push(new pageButton('查看审批流','process','/busiQualityMortarInspection/process','processData'));
		/**
		 * 设置界面的功能按钮.
		 */
	    ...
	    $scope.execute = function(operation, param){
	        ...
	        if(operation == 'viewData'){
	        	if(param.disposeState=='5' || param.disposeState=='4'){
	        		listService.editData($scope,$scope.gridApi, new popup("查看","/busiQualityMortarInspection/view", 'mortar_inspection', "1200", "800"),param);
	        	}else if(param.disposeState=='1'||param.disposeState=='2'||param.disposeState=='6'){
	        		listService.editData($scope,$scope.gridApi, new popup("查看","/busiQualityQuickProcess/detail", 'mortar_inspection', "1200", "800"),param);
	        	}else{
	        	...
	        	if(isAdminFn()){
	        		...
	        		for(var i=0;i<selectData.length;i++){
	        			...
	        			if (selectData[i].disposeState!='0' && selectData[i].disposeState!='6' || selectData[i].isQualified!='1') {
	 					submitTips('该条数据已提交整改或为合格数据，不能快捷处理！', 'warning');
	 					ids = [];
	 					break;
	 				}
	        		...
	        		if(ids.length>0){
		        		...
		        		listService.addData($scope,new popup("快捷处理","/busiQualityMortarInspection/quickProcess", map, "900", "600"));
	        		}
	        	}
	        	...
	        	if(isAdminFn()){
	        		...
	        		var ids = [];
	        		for(var i=0;i<selectData.length;i++){
	        			if (selectData[i].disposeState!='0' || selectData[i].isQualified!='1') {
	        		...
	        		if(ids.length>0){
	    	        	...
	    	        	listService.addData($scope,new popup("整改下发","/busiQualityMortarInspection/rectification", map, "1200", "800"));
	        		}
	        	}
	        	...
	        	window.open(param['descriptionUrl'],"检测报告");
	    	}
	    }
	    /**
		 * 初始化加载数据.
	    ...
	    var load = $scope.load = function() {
	        listService.loadData($scope, '/api/busiQualityMortarInspection/query', $scope.tableGrid);
	    }
 ...
 * 查询整改明细
 */
angular.module('huatek.controllers').controller('BusiQualityMortarInspectionDetailController', function ($scope,$rootScope, $location, $http,listService, editService, cacheService,httpService,$q) {
	var originType = $scope.passParams;
	var typeField = '';
	/* 判断接口来源类型 */
	if(originType=='mortar_inspection'){
		typeField = [
			{ name: '标段名称', field: 'orgName',width: '10%', enableColumnMenu: false},
			{ name: '样品编号', field: 'sampleCode',width: '10%', enableColumnMenu: false},
			{ name: '样品名称', field: 'sampleName',width: '10%', enableColumnMenu: false},
...
angular.module('huatek.controllers').controller('BusiQualityMortarInspectionRectificationController', function ($scope, $location, $http, editService, cacheService,$rootScope) {
	var addDataUrl = '/api/busiQualityMortarInspection/rectificationAdd';
	var homeUrl = "/busiQualityMortarInspection/home";
    /*定义块*/
    var columnWrapArray = [];
    ...
    contractFileUUID.systemHeader = "/root/quality/semifinished/mortarInspection";
 ...
 * 试验检测快捷处理
 */
angular.module('huatek.controllers').controller('BusiQualityMortarInspectionQuickProcessController', function ($rootScope, $scope, $http, listService, editService,httpService) {
	console.log($scope.passParams)
	var addDataUrl = '/api/busiQualityMortarInspection/quickProcessAdd';
    ...
    */
	 ...
	 * 初始化加载数据.
	 */
   /* httpService.get($scope,'/api/busiQualityMortarInspection/queryList'+"/"+$scope.passParams.id).success(function(response){
    	$scope.tableGrid.data = response;
    });*/
