...
import com.huatek.frame.core.beancopy.converter.ArrayConverter;
import com.huatek.frame.core.beancopy.converter.BigDecimalConverter;
import com.huatek.frame.core.beancopy.converter.BigIntegerConverter;
import com.huatek.frame.core.beancopy.converter.BooleanConverter;
import com.huatek.frame.core.beancopy.converter.ByteConverter;
import com.huatek.frame.core.beancopy.converter.CalendarConverter;
import com.huatek.frame.core.beancopy.converter.CharacterConverter;
import com.huatek.frame.core.beancopy.converter.ClassConverter;
import com.huatek.frame.core.beancopy.converter.ConverterFacade;
import com.huatek.frame.core.beancopy.converter.DateConverter;
import com.huatek.frame.core.beancopy.converter.DateTimeConverter;
import com.huatek.frame.core.beancopy.converter.DoubleConverter;
import com.huatek.frame.core.beancopy.converter.FileConverter;
import com.huatek.frame.core.beancopy.converter.FloatConverter;
import com.huatek.frame.core.beancopy.converter.IntegerConverter;
import com.huatek.frame.core.beancopy.converter.LongConverter;
import com.huatek.frame.core.beancopy.converter.NumberConverter;
import com.huatek.frame.core.beancopy.converter.ShortConverter;
import com.huatek.frame.core.beancopy.converter.SqlDateConverter;
import com.huatek.frame.core.beancopy.converter.SqlTimeConverter;
import com.huatek.frame.core.beancopy.converter.SqlTimestampConverter;
import com.huatek.frame.core.beancopy.converter.StringConverter;
import com.huatek.frame.core.beancopy.converter.URLConverter;
 ...
 * @version $Revision: 745079 $ $Date: 2009-02-17 14:04:10 +0000 (Tue, 17 Feb
 *          2009) $
 * @since 1.7
 */
...
public class ConvertUtilsBean {
	...
	private static ConvertUtilsBean instance = null;
	...
	private List<Converter> totalConverters = new ArrayList<Converter>();
	/**
	...
	public Object convert(Object value, Class targetType,String pattern) {
		...
		if (converter != null) {
			...
			boolean oldUseLocaleFormat=false;
		    
			if(pattern!=null){
				if(converter instanceof NumberConverter){
					NumberConverter numberConverter=(NumberConverter)converter;
					oldPattern=numberConverter.getPattern();
					oldUseLocaleFormat=numberConverter.getUseLocaleFormat();
					...
					oldPattern=dateTimeConverter.getPattern();
					oldUseLocaleFormat=dateTimeConverter.getUseLocaleFormat();
			...
			if(pattern!=null){
				if(converter instanceof NumberConverter){
					NumberConverter numberConverter=(NumberConverter)converter;
					numberConverter.setPattern(oldPattern);
					numberConverter.setUseLocaleFormat(oldUseLocaleFormat);
				}else if(converter instanceof DateTimeConverter){
					DateTimeConverter dateTimeConverter=(DateTimeConverter)converter;
					dateTimeConverter.setPattern(oldPattern);
					dateTimeConverter.setUseLocaleFormat(oldUseLocaleFormat);	
				}
			}
	...
	private void registerArrays(boolean throwException, int defaultArraySize) {
		...
		registerArrayConverter(Byte.TYPE, new ByteConverter(), throwException,
				defaultArraySize);
		...
		registerArrayConverter(Integer.TYPE, new IntegerConverter(),
				throwException, defaultArraySize);
		registerArrayConverter(Long.TYPE, new LongConverter(), throwException,
				defaultArraySize);
		...
		registerArrayConverter(Boolean.class, new BooleanConverter(),
				throwException, defaultArraySize);
		registerArrayConverter(Byte.class, new ByteConverter(), throwException,
				defaultArraySize);
		...
		registerArrayConverter(Integer.class, new IntegerConverter(),
				throwException, defaultArraySize);
		registerArrayConverter(Long.class, new LongConverter(), throwException,
				defaultArraySize);
	...
	private void registerArrayConverter(Class componentType,
			Converter componentConverter, boolean throwException,
			int defaultArraySize) {
		totalConverters.add(componentConverter);
	...
	public void addConvertParam(ConvertParam key, Object value) {
		if (ConvertParam.dateFormatPatten.equals(key)) {
			String dateFormatter = String.valueOf(value);
			for (Converter converter : totalConverters) {
				if (converter instanceof DateTimeConverter) {
					DateTimeConverter dateTimeConverter = (DateTimeConverter) converter;
					dateTimeConverter.setPattern(dateFormatter);
					dateTimeConverter.setUseLocaleFormat(true);

				}
			}

		} else if (ConvertParam.trueValue.equals(key)) {
			String trueValue = String.valueOf(value);
			for (Converter converter : totalConverters) {
				if (converter instanceof BooleanConverter) {
					BooleanConverter booleanConverter = (BooleanConverter) converter;
					booleanConverter.setTrueString(trueValue);
					;

				}
				if (converter instanceof NumberConverter) {
					NumberConverter numberConverter = (NumberConverter) converter;
					numberConverter.setTrueString(trueValue);
					;

				}
			}
		} else if (ConvertParam.falseValue.equals(key)) {
			String falseValue = String.valueOf(value);
			for (Converter converter : totalConverters) {
				if (converter instanceof BooleanConverter) {
					BooleanConverter booleanConverter = (BooleanConverter) converter;
					booleanConverter.setFalseString(falseValue);
					;

				}
				if (converter instanceof NumberConverter) {
					NumberConverter numberConverter = (NumberConverter) converter;
					numberConverter.setFalseString(falseValue);
					;
