    ...
    methods: {
          ...
          if (this.filterData.category && this.filterData.category.length) {
            this.filterData.category.forEach((item, index) => {
              if (item.code === this.$route.query.category) {
                ...
                this.seletedFilter.items.push(item.code)
              }
            })
            this.queryParamList.push(this.seletedFilter)
          ...
          .then(res => {
            ...
            res.forEach((item, index) => {
              ...
              if (index === 0) {
                que.field = 'score'
                que.logic = '<='
                que.value = item.value
                item.queryParamList.push(que)
              }
              if (index === 1) {
                ...
                query.logic = '<='
                query.value = val[1]
                item.queryParamList.push(query, que)
              }
              if (index === 2) {
                que.field = 'score'
                que.logic = '>='
            ...
            console.log(this.filterData.recommend)
          })
      },
      getCategory () {
      ...
      selecteVal (val, index, code) {
        if (val === 1) { // category
          ...
          if (i !== -1) {
            this.seletedFilter.items.splice(i, 1)
            this.categoryCurIndexs.splice(i, 1)
          } else {
            this.seletedFilter.items.push(code)
            this.categoryCurIndexs.push(index)
          }
        } else {
          ...
          this.selectedRecommend = this.filterData.recommend[index].queryParamList
        }
      },
      confirmFilter () {
        this.filterVisible = false
        this.queryParamList = []
        if (this.seletedFilter.items.length) {
          this.queryParamList.push(this.seletedFilter)
        }
        if (this.selectedRecommend) {
          this.queryParamList = [...this.queryParamList, ...this.selectedRecommend]
        }
        console.log(this.queryParamList)
